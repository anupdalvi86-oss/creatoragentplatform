...
The owner chose the 32 roles and explicit exclusions in [AGENT_CATALOG.md](AGENT_CATALOG.md). Treat them as a **backlog**, not a request to scaffold 32 empty agents. Start with a creator-scoped admin task/run foundation and one useful read-only or reversible role. Once the shared contract is tested, work on content/transcripts, consumer agents, Hermes engineering workflows and operations in parallel where independent branches and integration capacity allow. Draft/review gates protect newly ingested creator content; they are not a permanent restriction on ordinary agent responses. See the catalog for role-by-role status and acceptance criteria and the [Hermes handoff](HERMES_HANDOFF.md) for the complete task prompt. New work must use additive migrations, protect existing routes and pass the full local quality gate. Provisioning remote infrastructure, pushing and deploying require separate approval.
