...
## Selected specialist-agent program (32-role onboarding fan-out implemented)

The owner chose the 32 roles and explicit exclusions in [AGENT_CATALOG.md](AGENT_CATALOG.md). The shared creator-scoped task/run foundation is implemented in migrations `0003`–`0006`, with an allowlisted role registry, idempotency, bounded status transitions, audit runs, Data Quality Monitor execution, a reversible Content Librarian preview and durable handoff lineage. `POST /api/admin/creator-setup` now submits all 32 selected handlers idempotently. Some tasks intentionally finish as `awaiting_review` or an explicit integration-required result: this is expected for rights, transcripts, provider access and engineering plans. The Admin Operations panel exposes the role/task/finding state. Draft/review gates protect newly ingested creator content; they are not a permanent restriction on ordinary agent responses. See the catalog for role-by-role boundaries and the [Hermes handoff](HERMES_HANDOFF.md) for the engineering separation. New work must use additive migrations, protect existing routes and pass the full local quality gate.
