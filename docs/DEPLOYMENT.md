...
Apply additive migration `0007_fitness.sql` to the target D1 database only after owner approval. Fitness studio requests use `/api/admin/fitness-studio/<slug>/*`, within the existing Cloudflare Access application for `/api/admin/*`. The Worker validates the configured Access JWT audience and creator email membership for each studio request; cooking admin roles remain separate. The PWA's Creator sign in link starts the Access login and returns to the creator page. The owner confirmed exercise-video use in this app on 2026-09-25; see [fitness media policy](FITNESS_MEDIA_POLICY.md). This approval does not cover bulk rehosting or unrelated media. Browser reminders and opted-in creator follow-ups run only while the PWA is open; background push is not configured. Premium programs are gated on the server through fitness-specific entitlements, but no checkout or grant integration exists. Confirm the target domain, Access policy and migration state before deployment.

For a fitness tenant, `/admin?advanced=1&creator=<slug>` opens a dedicated fitness admin view. Workouts links to the creator's program editor; Review lets an owner approve or return submitted programs and challenges; Community shows creator challenges and moderatable posts; Settings manages branding and Access email membership. The creator saves a draft, submits it for review, and publishes only after approval. Activation of a fitness tenant requires a published program, rather than cooking source content. The fitness summary API exposes aggregate workout completions and participant counts only; it does not return body measurements.
