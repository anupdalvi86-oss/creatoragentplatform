...
For new product tasks, implement an allowlisted role identifier, typed payload/result, creator and operator scope, idempotency key, queued/running/awaiting_review/completed/failed states, retry bounds, time/token/cost caps, approval events and error codes. Persist source IDs and rights provenance, not raw prompts in logs. The first run must be draft-only. Treat a missing transcript provider, unavailable queue or model rate limit as a visible task outcome, not proof of success. Keep external side effects out of retries unless idempotent.
