...
The platform now has a bounded specialist-task foundation, but it is **not yet a general multi-agent runtime**. Migration `0003_agent_tasks.sql` provides the role registry, creator-scoped tasks, idempotency, approval-aware status, run audit records and data-quality findings. The Worker currently executes the deterministic Data Quality Monitor and a reversible Content Librarian preview; the registered consumer/content roles still need their own adapters and tests. The cooking flow remains a fixed retrieval/answer path, and Hermes remains the engineering environment rather than a Worker sub-process. The planned role contracts and status are in [AGENT_CATALOG.md](AGENT_CATALOG.md).
