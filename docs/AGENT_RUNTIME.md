...
`POST /api/admin/creator-setup` is the one-link onboarding trigger. It provisions or reuses a cooking tenant, imports supported public metadata, submits one idempotent task for each of the 32 selected handlers, and returns the PWA path with task status. Engineering handlers produce review-only plans/checklists; they do not edit the repository, deploy, publish, change payments or invoke Hermes. Provider-dependent handlers report the exact missing authorization or integration, and explicitly excluded roles are never submitted.
