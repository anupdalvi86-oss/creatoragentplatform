...
The platform now has a bounded specialist-task runtime with durable handoff support. Migrations `0003`–`0006` provide the role registry, creator-scoped tasks, idempotency, approval-aware status, run audit records, data-quality findings and parent/child handoff records. All 32 selected non-excluded roles have typed handlers. Standalone tasks are delivered through the task Queue; chained tasks run as Cloudflare Workflow instances; approvals resume a paused chain; and an hourly schedule starts Data Quality Monitor and Cost Monitor jobs. Deterministic product roles execute against tenant-scoped data; content and engineering effects produce reviewable drafts; provider-dependent roles report explicit integration requirements instead of fabricating work. The cooking flow remains a fixed retrieval/answer path, and Hermes remains the engineering environment for repository changes rather than a Worker sub-process. The role contracts and status are in [AGENT_CATALOG.md](AGENT_CATALOG.md).

`POST /api/admin/creator-setup` is the one-link onboarding trigger. It provisions or reuses a cooking or fitness tenant and imports supported public metadata. Cooking setup submits the selected specialist handlers; fitness setup uses the dedicated catalog/program workflow and runs Scout without dispatching cooking handlers. Engineering handlers produce review-only plans/checklists; they do not edit the repository, deploy, publish, change payments or invoke Hermes. Provider-dependent handlers report the exact missing authorization or integration, and explicitly excluded roles are never submitted.

### Automatic handoff

Submit a root task with `input.workflow.steps`, where each step is either a role key or `{ roleKey, input }`. The Worker creates the first task, dispatches it automatically, persists each parent/child handoff in `agent_task_handoffs`, and passes the previous structured result into the next task as `input.handoff`. A completed step advances automatically; an `awaiting_review` step pauses the chain. Approve it through `POST /api/admin/agent-tasks/:creatorId/:taskId/approve`, and the next step resumes. Inspect lineage through `GET /api/admin/agent-tasks/:creatorId/:taskId/handoffs`.
...
Fitness creator setup uses deterministic program and catalog workflows rather than cooking agent tools. Existing specialist roles are not dispatched automatically for fitness because their current handlers assume cooking content. Creator programs and challenges require administrator review before the creator may publish them. No new agent role or automatic release authority was added.
