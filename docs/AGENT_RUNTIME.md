...
The platform now has a bounded specialist-task runtime, but it is **not an unconstrained autonomous multi-agent system**. Migrations `0003`–`0005` provide the role registry, creator-scoped tasks, idempotency, approval-aware status, run audit records and data-quality findings. All 32 selected non-excluded roles have typed handlers. Deterministic product roles execute against tenant-scoped data; content and engineering effects produce reviewable drafts; provider-dependent roles report explicit integration requirements instead of fabricating work. The cooking flow remains a fixed retrieval/answer path, and Hermes remains the engineering environment rather than a Worker sub-process. The role contracts and status are in [AGENT_CATALOG.md](AGENT_CATALOG.md).
