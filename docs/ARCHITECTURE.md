...
The first product slice should add an authenticated task submission path, persisted task/run lifecycle, allowlisted role/tool registry and a useful read-only or reversible operation. Once those interfaces are tested, separate content, customer-facing, engineering and operations tracks can advance concurrently and integrate independently. Authorized content intake can fan out through a queue into a durable transcript/extract/review workflow; reviewed source records then feed tenant-scoped retrieval. Queue, workflow, upload storage and dedicated audio processing are **design options**, not current bindings. Add them only after verifying workload, Cloudflare account resources, permissions and cost. A failed/retried job must not duplicate content or bypass review. Draft/review requirements attach to new unapproved creator content, not ordinary bounded customer answers.
