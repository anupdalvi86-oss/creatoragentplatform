...
Fitness uses the existing Worker and `creators` table with `category='fitness'`, a separate React view and tenant-specific manifest. Cooking routes reject fitness tenants; fitness routes reject cooking tenants. Creator studio calls use `/api/admin/fitness-studio/:slug/*` inside the existing Cloudflare Access policy and require a verified Access JWT mapped to `fitness_creator_members` or an operator role. The admin workspace renders fitness-specific Workouts, Review, Community and Settings controls for fitness tenants; it does not expose cooking agent actions there. Fitness audience cookies are signed with a fitness-specific scope and backed by `fitness_users`, separate from cooking `users`. The static shared exercise adapter retains catalog identifiers, metadata provenance and remote demonstration URLs. Program rows reference exercise identifiers instead of duplicating records. No new deployed specialist role or background infrastructure is introduced.
