...
For another cooking creator, paste one YouTube, Instagram, or other creator profile/channel link into `/admin` and choose **Build PWA**. The setup creates or reuses the tenant, starts the applicable creator-facing content and operations agents, and returns the PWA URL. YouTube imports up to 25 public metadata cards; Instagram and unsupported sources report the exact authorization/connector requirement. Transcripts, media and derived recipe steps still require creator authorization and review. Engineering, commercial and explicitly excluded roles are not launched as consumer onboarding jobs. Operator-supplied recipe metadata starts in review and must be inspected before it is marked ready. The same Worker and PWA serve every slug. For another vertical, add a vertical-specific interface and tool definitions before enabling that category; see [architecture](docs/ARCHITECTURE.md).
