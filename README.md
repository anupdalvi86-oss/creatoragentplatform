...
The Build PWA form detects a YouTube channel's vertical from official public metadata when available. Mixed or unavailable signals require an explicit administrator choice. A fitness tenant has its own branded manifest, creator studio, program review/publishing flow, anonymous audience sessions, onboarding, routine matching, workout tracking, challenges and discussion. The protected admin workspace shows fitness-specific Overview, Workouts, Review, Community and Settings tabs instead of cooking agents and meal tools. Open `/admin?advanced=1&creator=<fitness-slug>` and choose **Workouts → Add a workout program**. In the creator studio, name the program, set its goal, level, equipment, session length and days, search and preview exercises, add them to each day, set reps or duration and rest, and save the draft. Submit it for review; an owner approves it in the admin Review tab, then publishes it from Workouts. The creator must have a verified Cloudflare Access email membership to edit a program; owners can assign membership in Settings. A fitness user never shares a session or profile with a cooking user, even if other identifying details coincide.
